# 版本控制说明

## 概述

CursorToolset 从 v1.0.0 开始支持智能版本控制，可以：
- 检查是否有新版本
- 只在需要时更新，避免不必要的下载和构建
- 对比本地和远程版本号

## 版本号管理

### 单一数据源原则

**所有版本号都从 `version.json` 文件读取**，这是版本号的唯一来源。

```json
{
  "version": "v1.0.0",
  "build_time": "",
  "commit": "",
  "branch": ""
}
```

详细说明请查看 [VERSION_MANAGEMENT.md](./VERSION_MANAGEMENT.md)。

### 版本号格式

采用语义化版本号（Semantic Versioning）：`vMAJOR.MINOR.PATCH`

例如：
- `v1.0.0` - 主版本 1，次版本 0，补丁版本 0
- `v1.2.3` - 主版本 1，次版本 2，补丁版本 3
- `v2.0.0` - 主版本 2（重大更新）

## 版本比较逻辑

### 比较规则

1. **主版本号**（Major）最重要，优先比较
2. **次版本号**（Minor）其次
3. **补丁版本号**（Patch）最后

示例：
```
v2.0.0 > v1.9.9  (主版本号更大)
v1.2.0 > v1.1.9  (次版本号更大)
v1.0.2 > v1.0.1  (补丁版本号更大)
v1.0.0 = v1.0.0  (完全相同)
```

### 特殊版本

- `dev` - 开发版本，总是被认为需要更新
- `unknown` - 未知版本，同 `dev`

## 更新机制

### 1. 自我更新 (`--self`)

```bash
cursortoolset update --self
```

**流程**：
1. 🔍 从 GitHub API 获取最新 Release 版本号
2. 📌 显示当前版本和最新版本
3. 🔄 比较版本号
4. ✅ 如果已是最新版本，跳过更新
5. 🆕 如果有新版本，执行更新

**示例输出**：
```
🔄 更新 CursorToolset...
  🔍 检查新版本...
  📌 当前版本: v1.0.0
  📌 最新版本: v1.1.0
  🆕 发现新版本！
  📍 当前位置: /Users/xxx/.cursor/toolsets/CursorToolset/bin/cursortoolset
  📥 克隆最新代码...
  🔨 构建新版本...
  📦 替换旧版本...
  ✅ CursorToolset 更新完成
```

如果已是最新：
```
🔄 更新 CursorToolset...
  🔍 检查新版本...
  📌 当前版本: v1.1.0
  📌 最新版本: v1.1.0
  ✅ 已是最新版本，无需更新
✅ CursorToolset 更新完成
```

### 2. 配置文件更新 (`--available`)

```bash
cursortoolset update --available
```

**流程**：
1. 📥 从 GitHub 下载最新 `available-toolsets.json`
2. 🔍 比较本地和远程文件内容
3. ✅ 如果内容相同，跳过更新
4. 🆕 如果有变化，替换本地文件

**示例输出**：
```
🔄 更新 available-toolsets.json...
  📍 配置文件: /Users/xxx/.cursor/toolsets/CursorToolset/available-toolsets.json
  🔍 检查配置文件更新...
  📥 下载最新配置...
  ✅ 配置文件已是最新，无需更新
```

### 3. 工具集更新 (`--toolsets`)

```bash
cursortoolset update --toolsets
```

**流程**：
1. 🔄 遍历所有已安装的工具集
2. 📥 执行 `git fetch` 检查远程更新
3. 🔍 检查本地是否落后于远程（`git status`）
4. ✅ 如果已是最新，跳过
5. 🆕 如果有更新，执行 `git pull`

**示例输出**：
```
🔄 更新已安装的工具集...
  🔄 检查 GitHub Action AI 工具集...
    ✅ 已是最新版本

  🔄 检查 另一个工具集...
    🆕 发现新版本，正在更新...
    ✅ 更新成功

  📊 更新统计: 成功 1 个
✅ 所有工具集更新完成
```

## 版本检查 API

### 查看当前版本

```bash
cursortoolset --version
# 输出: cursortoolset version v1.0.0 (built at 2024-12-04_11:00:00)
```

**版本号来源**：程序会从 `version.json` 读取版本号（如果编译时未注入）。

### 编程接口

CursorToolset 提供了版本比较的 Go 包：

```go
import "github.com/firoyang/CursorToolset/pkg/version"

// 比较两个版本
result := version.Compare("v1.0.0", "v1.0.1")
// result < 0: v1.0.0 < v1.0.1
// result = 0: 相等
// result > 0: v1.0.0 > v1.0.1

// 检查是否需要更新
needUpdate := version.NeedUpdate("v1.0.0", "v1.1.0")
// needUpdate = true

// 获取 GitHub 最新版本
release, err := version.GetLatestRelease("firoyang", "CursorToolset")
if err == nil {
    fmt.Println("最新版本:", release.TagName)
    fmt.Println("Release 页面:", release.HTMLURL)
}
```

## 构建时注入版本号

### 使用 Makefile

```bash
make build       # 使用 Git 标签作为版本号
make build-all   # 构建所有平台，包含版本信息
```

### 手动构建

```bash
# Makefile 自动从 version.json 读取版本号
make build

# 或手动指定
VERSION=$(cat version.json | grep -o '"version"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
BUILD_TIME=$(date -u '+%Y-%m-%d_%H:%M:%S')
go build -ldflags "-X main.Version=$VERSION -X main.BuildTime=$BUILD_TIME" -o cursortoolset
```

**重要**：版本号必须从 `version.json` 读取，不要硬编码或从其他来源获取。

### GitHub Actions 自动构建

在 `.github/workflows/build.yml` 和 `.github/workflows/release-process.yml` 中：
- 从 `version.json` 读取版本号
- 自动填充 `build_time`、`commit`、`branch` 字段
- 构建多平台版本
- 创建 GitHub Release

**版本号来源**：始终从 `version.json` 读取，确保一致性。

## 版本发布流程

### 1. 开发阶段
```bash
# 开发中的版本号
./cursortoolset --version
# 输出: cursortoolset version dev
```

### 2. 准备发布
```bash
# 1. 更新 CHANGELOG.md
# 2. 提交所有更改
git add .
git commit -m "chore: prepare for v1.1.0"

# 3. 创建 Git 标签
git tag -a v1.1.0 -m "Release v1.1.0"

# 4. 推送标签到远程
git push origin v1.1.0
```

### 3. 自动构建
- GitHub Actions 自动触发
- 构建所有平台版本
- 创建 GitHub Release
- 上传预编译文件

### 4. 用户更新
```bash
# 用户执行更新
cursortoolset update --self

# 系统自动检测到新版本并更新
```

## 网络问题处理

如果无法连接 GitHub API：

```
🔄 更新 CursorToolset...
  🔍 检查新版本...
  ⚠️  无法检查版本: 请求失败
  ℹ️  继续尝试更新...
```

系统会跳过版本检查，直接执行更新。

## 最佳实践

### 用户
1. **定期更新**：建议每月运行 `cursortoolset update`
2. **查看版本**：安装后先运行 `cursortoolset --version` 确认
3. **网络问题**：如遇 GitHub 访问问题，可以手动从源码构建

### 开发者
1. **语义化版本**：遵循 SemVer 规范
2. **Git 标签**：每个版本都打 tag
3. **CHANGELOG**：记录每个版本的变更
4. **测试**：发布前充分测试

## 故障排除

### 问题 1: 版本检查失败

**错误**：`无法检查版本: 请求失败，状态码: 403`

**原因**：GitHub API 请求限制

**解决**：
- 等待一段时间后重试
- 使用 GitHub Token（环境变量 `GITHUB_TOKEN`）

### 问题 2: 更新后版本号不变

**原因**：使用的是开发版本，未注入版本号

**解决**：
```bash
# 使用 Makefile 构建
make build

# 或手动指定版本
go build -ldflags "-X main.Version=v1.0.0" -o cursortoolset
```

### 问题 3: 无法获取最新版本

**原因**：GitHub 上没有 Release

**解决**：
- 确保在 GitHub 上创建了 Release
- 或从 `main` 分支构建

## 参考资源

- [语义化版本规范](https://semver.org/lang/zh-CN/)
- [GitHub Releases](https://docs.github.com/en/repositories/releasing-projects-on-github)
- [Go Build 参数注入](https://pkg.go.dev/cmd/go#hdr-Compile_packages_and_dependencies)

